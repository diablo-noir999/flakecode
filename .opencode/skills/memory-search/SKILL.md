...
- Text search returned nothing useful despite multiple query attempts.

## Locating the database

```bash
# Typically at this path. Check environment for overrides.
...
- `{"type":"tool","tool":"<name>","callID":"...","state":{"status":"error","input":{...},"error":"..."}}` — failed tool call
...
- `agent_id = 'main'` = main agent; other values = subagent.
...
**Find repeated errors (last 7 days):**
...
| Verify a memory claim | Find the session_id from the memory entry, then query its full execution chain |
