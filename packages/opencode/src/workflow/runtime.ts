...
import { Instance } from "@/project/instance-context"
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const resolveAgentModel = (
      ...
      fallback: { providerID: ProviderV2.ID; modelID: ModelV2.ID } | undefined,
      warned: Set<string>,
      ...
      ref === undefined
        ...
        : Effect.gen(function* () {
            ...
            const [providerID, modelID] = parts as [ProviderV2.ID, ModelV2.ID]
            const result = yield* provider.getModel(providerID, modelID).pipe(Effect.catchCause(() => Effect.succeed(null)))
            if (!result) {
              if (!warned.has(ref)) {
                warned.add(ref)
                log.warn("workflow agent model ref did not resolve — using run default", { ref })
    ...
    let cfg: ConfigV1.Info | undefined
    let globalMax = 0
    ...
    const ensureGlobal = Effect.fn("WorkflowRuntime.ensureGlobal")(function* () {
      ...
      globalMax = Math.max(
        1,
        Math.min(DEFAULT_MAX_CONCURRENT, 2 * Math.max(1, cpuCount())),
      )
        ...
        setTimeout(() => {
          ...
          Effect.runFork(
            WorkflowPersistence.flushCounters({
              ...
              succeeded: entry.succeeded,
              failed: entry.failed,
            }).pipe(Effect.ignore) as Effect.Effect<void, never, never>,
          )
        }, 250),
      )
    }
    ...
    const reclaim = (entry: RunEntry): Effect.Effect<void, never, any> =>
      Effect.gen(function* () {
    ...
    const launch = Effect.fn("WorkflowRuntime.launch")(function* (input: StartInput, runID: string, name: string) {
      ...
      const lifecycleCap = input.maxLifecycleAgents ?? MAX_LIFECYCLE_AGENTS
      ...
      const spawnShared = async (
        ...
        prompt: string,
        o: AgentOpts,
        resolvedModel: { providerID: ProviderV2.ID; modelID: ModelV2.ID } | undefined,
      ...
      const spawnIsolated = async (
        ...
        const wtCtx = await Instance.provide({
          ...
          fn: async () => await Instance.current(),
        })
        const wtBridge = await bridge.promise(EffectBridge.make().pipe(Effect.provideService(InstanceRef, wtCtx as any)))
      ...
      const agentImpl = (prompt: unknown, opts?: unknown, nodeId?: string) => {
        ...
        if (o.isolation !== "worktree") {
          ...
          if (journal.results.has(key)) {
            ...
            // reflects replay as replay proceeds.
            entry.succeeded++
      ...
      const phase: HostFn = (title: unknown) => {
        ...
        Effect.runFork(WorkflowPersistence.recordPhase({ runID, phase: String(title) }).pipe(Effect.ignore) as Effect.Effect<void, never, never>)
        Effect.runFork(WorkflowPersistence.appendJournal(runID, { t: "phase", title: String(title), pass }).pipe(Effect.ignore) as Effect.Effect<void, never, never>)
        Effect.runFork(bus.publish(WorkflowPhase, { sessionID: input.sessionID, runID, title: String(title) }) as Effect.Effect<void, never, never>)
        return undefined
      }

      const logHook: HostFn = (message: unknown) => {
        entry.transcript.push({ kind: "log", text: String(message) })
        Effect.runFork(WorkflowPersistence.appendJournal(runID, { t: "log", msg: String(message), pass }).pipe(Effect.ignore) as Effect.Effect<void, never, never>)
        Effect.runFork(bus.publish(WorkflowLog, { sessionID: input.sessionID, runID, message: String(message) }) as Effect.Effect<void, never, never>)
        return undefined
      }
      ...
      entry.fiber = yield* work.pipe(Effect.forkIn(scope)) as any
      return { runID }
    })
    ...
    const start = Effect.fn("WorkflowRuntime.start")(function* (input: StartInput) {
      ...
      const runID = Identifier.descending("job")
    ...
    const impl = Service.of({ start, status, wait, transcript, structure, cancel, list, resume } as any)
...
export const defaultLayer = layer.pipe(
  ...
  Layer.provide(Worktree.node.implementation as Layer.Layer<never>),
