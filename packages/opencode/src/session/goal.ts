...
export interface Interface {
  readonly set: (sessionID: SessionID, condition: string) => Effect.Effect<void, unknown>
  readonly get: (sessionID: SessionID) => Effect.Effect<Goal | undefined, unknown>
  readonly clear: (sessionID: SessionID) => Effect.Effect<void, unknown>
  /** Increment the re-entry counter, returning the new count. */
  readonly bumpReact: (sessionID: SessionID) => Effect.Effect<number, unknown>
  /**
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const elog = Log.create({ service: "SessionGoal" })
    ...
    const set = Effect.fn("SessionGoal.set")(function* (sessionID: SessionID, condition: string) {
      const data = yield* InstanceState.get(state).pipe(Effect.orDie)
      data.goals.set(sessionID, { condition, react: 0 })
      elog.info("goal set", { sessionID, condition })
    ...
    const clear = Effect.fn("SessionGoal.clear")(function* (sessionID: SessionID) {
      ...
      elog.info("goal cleared", { sessionID })
    ...
    const evaluate = Effect.fn("SessionGoal.evaluate")(function* (input: {
      ...
      elog.debug("goal judge transcript", {
