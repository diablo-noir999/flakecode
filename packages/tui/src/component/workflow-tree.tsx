...
type WorkflowNode = WorkflowPhase | WorkflowAgent | WorkflowChild
...
function agentMeta(n: WorkflowAgent) {
  const parts: string[] = []
...
function WorkflowCard(props: { node: WorkflowChild; onOpenChild?: (childRunID: string) => void }) {
  const { theme } = useTheme()
  const [hover, setHover] = createSignal(false)
  const spine = () =>
            ...
            <Show when={node.type === "phase"}>
              <box marginTop={1} flexDirection="row" gap={1} alignItems="center">
                <text attributes={TextAttributes.BOLD} fg={theme.accent}>
                  ◆ {(node as WorkflowPhase).title}
                </text>
                <box flexGrow={1} borderColor={theme.borderSubtle} border={["bottom"]} />
              </box>
            </Show>
            <Show when={node.type === "agent"}>
              <AgentCard node={node as WorkflowAgent} onOpenAgent={props.onOpenAgent} liveActivity={props.liveActivity} />
            </Show>
            <Show when={node.type === "workflow"}>
              <WorkflowCard node={node as WorkflowChild} onOpenChild={props.onOpenChild} />
            </Show>
          </>
        )}
      </For>
