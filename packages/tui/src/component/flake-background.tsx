...
const FLAKE_TEAL = RGBA.fromInts(0, 209, 209)
const FLAKE_GREEN = RGBA.fromInts(0, 255, 183)
const FLAKE_YELLOW = RGBA.fromInts(251, 255, 0)
...
function generateField(w: number, h: number): FlakeField {
  const grid: string[][] = []
  ...
  for (let y = 0; y < h; y++) {
    ...
    for (let x = 0; x < w; x++) {
      if (Math.random() < DENSITY) {
        const idx = Math.floor(Math.random() * (FLAKE_CHARS.length - 1))
        row.push(String(idx))
...
function flakeColorForBrightness(brightness: number): RGBA {
  if (brightness < 0.33) return FLAKE_TEAL
  if (brightness < 0.66) return FLAKE_GREEN
  return FLAKE_YELLOW
}

export function FlakeBackground(props: { animated?: () => boolean; meteor?: () => boolean; running?: () => boolean } = {}) {
  const { theme } = useTheme()
  const [field, setField] = createSignal<FlakeField>({ grid: [], brightness: [] })
  ...
  const content = createMemo(() => {
    ...
    if (m) {
      ...
      if (elapsed >= 0 && elapsed <= METEOR_DURATION) {
        ...
        const beamCore = FLAKE_TEAL
        const beamGlow = FLAKE_GREEN
    ...
    f.grid.forEach((row, y) => {
      row.forEach((cell, x) => {
        ...
        const char = isHot ? HOT_CHAR : FLAKE_CHARS[idx % (FLAKE_CHARS.length - 1)]
        const baseColor = tint(theme.background, flakeColorForBrightness(b), Math.min(1, b * 1.05))
        const starColor =
          peak > 0
            ? tint(baseColor, FLAKE_YELLOW, peak * 0.65)
            : baseColor
  ...
  return (
    ...
    >
      ...
      <Show when={props.running?.()}>
        <Spaceship running={props.running} />
      </Show>
    </box>
  )
}
